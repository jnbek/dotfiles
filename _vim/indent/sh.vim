...
" Id:               $Date: 2016-03-28 19:00:22+09 $
"                   $Revision: 4.21 $
"
...
function GetShIndent()
  ...
  let ind = s:PrevLineIndent(line, pline, ind)
...
function s:PrevLineIndent(line, pline, ind)
  let ind = a:ind
...
function s:CaseLabelLineIndent(line, ind)
  ...
  if line =~ ')' && line =~ '^\s*('
    ...
    let ind = ind + &sw
  elseif line =~ ')'
    let line = strpart(line, matchend(line, ')', 0))
    let ind = ind + &sw
  endif
  if line =~ ';[;&]\s*$'
    let ind = ind - &sw
...
function s:BackQuoteIndent(lnum, ind)
  ...
  if line =~ '\\\@<!\%(\\\\\)*`'
    ...
    while sum > -1
      let esum = -1
      let esum = matchend(line, '`', sum)
      if esum > 0 && synIDattr(synID(a:lnum, esum, 1), "name") =~ s:back_quote
        let sum1 = 0
        let sum2 = 0
        let pnum = 0
        let psum = 0
        for cid in synstack(a:lnum, esum)
        ...
        if sum1 > sum2
          let ind += &sw
        elseif sum1 && sum1 == sum2 && sum == lsum
          let ind += &sw
